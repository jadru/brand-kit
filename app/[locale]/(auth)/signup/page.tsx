...
export default function SignupPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (password !== confirmPassword) {
      setError(t('validation.passwordsDoNotMatch', { defaultValue: tAuth('validation.passwordsDoNotMatch') }))
      return
    }

    if (password.length < 6) {
      setError(t('validation.passwordTooShort', { defaultValue: tAuth('validation.passwordTooShort') }))
      return
    }
        ...
        <div className="relative z-10 flex h-full flex-col justify-between p-12">
          ...
          <div className="animate-fade-in-up delay-3 flex gap-10">
            <div>
              ...
              <div className="mt-1 font-mono text-[10px] tracking-[0.2em] text-white/30 uppercase">{t('branding.stats.start')}</div>
            </div>
            <div>
              <div className="font-display text-2xl font-bold text-white tracking-tight">3/Month</div>
              <div className="mt-1 font-mono text-[10px] tracking-[0.2em] text-white/30 uppercase">{t('branding.stats.perMonth')}</div>
            </div>
          </div>
        </div>
      </div>
        ...
        <Card className="w-full max-w-md border-0 shadow-none lg:border lg:shadow-sm">
          <CardHeader>
            <div className="mb-2 lg:hidden">
              <Link href="/" className="font-display text-lg font-bold text-text-primary">BrandKit</Link>
            </div>
                ...
                {password.length > 0 && (
                  <div className="flex items-center gap-2 text-xs">
                    {passwordChecks.length ? (
                      <span className="flex items-center gap-1 text-success"><Check className="h-3 w-3" /> {t('passwordChecks.length')}</span>
                    ) : (
                      <span className="flex items-center gap-1 text-text-tertiary"><X className="h-3 w-3" /> {t('passwordChecks.length')}</span>
                    )}
                  </div>
                )}
              </div>
                ...
                {confirmPassword.length > 0 && (
                  <div className="flex items-center gap-2 text-xs">
                    {passwordChecks.match ? (
                      <span className="flex items-center gap-1 text-success"><Check className="h-3 w-3" /> {t('passwordChecks.match')}</span>
                    ) : (
                      <span className="flex items-center gap-1 text-error"><X className="h-3 w-3" /> {t('passwordChecks.mismatch')}</span>
                    )}
                  </div>
                )}
              </div>
